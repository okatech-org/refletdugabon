...
const Restaurant = () => {
  const { data: content } = useSiteContent("restaurant");
    ...
    <Layout>
      ...
      <section className="relative min-h-[60vh] flex items-center">
        <div className="absolute inset-0">
          <img
            src={getContent(content, "hero", "image", "") || restaurantImage}
            alt="Restaurant Les Délices du Gabon"
        ...
        <div className="relative section-container py-20">
          <div className="max-w-2xl text-primary-foreground">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gold/20 text-gold mb-4">
              ...
              {getContent(content, "hero", "badge", "Finaliste Cuistos Engagés 2022")}
            </span>
            <h1 className="text-4xl sm:text-5xl font-bold mb-6">
              {getContent(content, "hero", "title", "Les Délices du Gabon")}
            </h1>
            <p className="text-xl text-primary-foreground/80 mb-8">
              {getContent(content, "hero", "subtitle", "Gastronomie africaine authentique en Normandie, France.")}
            </p>
        ...
        <div className="section-container">
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="text-3xl font-bold text-foreground mb-6">
              {getContent(content, "about", "title", "Un Voyage Culinaire au Cœur de l'Afrique")}
            </h2>
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: getContent(
                  ...
                  "about",
                  "description",
                  "<p>Notre restaurant associatif vous invite à découvrir les saveurs authentiques du Gabon et de l'Afrique. Chaque plat raconte une histoire, celle de nos traditions culinaires transmises de génération en génération.</p><p>Reconnu pour notre approche écoresponsable, nous avons été finalistes du prix \"Les Cuistos Engagés\" organisé par MIIMOSA à Paris en juin 2022. Les bénéfices du restaurant soutiennent directement nos projets d'autonomisation au Gabon.</p>"
                ),
              }}
            />
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="bg-card p-8 rounded-2xl border border-border text-center">
              ...
              <p className="text-muted-foreground">
                {getContent(content, "contact", "address", "Verneuil-sur-Avre, Normandie, France")}
              </p>
            </div>
            <div className="bg-card p-8 rounded-2xl border border-border text-center">
              ...
              <a
                href={`tel:${getContent(content, "contact", "phone", "+33681657870")}`}
                className="text-muted-foreground hover:text-primary"
              >
                {getContent(content, "contact", "phone", "+33 6 81 65 78 70")}
              </a>
            </div>
            <div className="bg-card p-8 rounded-2xl border border-border text-center">
              ...
              <p className="text-muted-foreground">
                {getContent(content, "contact", "hours", "Sur réservation")}
              </p>
            </div>
          </div>
        </div>
