...
const Presidente = () => {
  const { data: content } = useSiteContent("presidente");
  ...
  return (
    <Layout>
          ...
          <div className="section-container">
            <div className="text-center max-w-3xl mx-auto">
              <span className="inline-block px-4 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-medium mb-4">
                {getContent(content, "hero", "badge", "Message de la Direction")}
              </span>
              <h1 className="text-4xl sm:text-5xl font-bold text-foreground mb-6">
                {getContent(content, "hero", "title", "Le Mot de la")}{" "}
                <span className="text-gradient-primary">
                  {getContent(content, "hero", "title_highlight", "Présidente")}
                </span>
              </h1>
            </div>
          </div>
      ...
      {/* Content */}
      {isSectionVisible(content, "message") && (
            ...
            <div className="max-w-4xl mx-auto">
              <div className="bg-card rounded-3xl border border-border p-8 sm:p-12 relative">
                ...
                <div className="relative space-y-6 text-muted-foreground leading-relaxed">
                  {getContent(content, "message", "content", "") ? (
                    <div
                      dangerouslySetInnerHTML={{
                        __html: getContent(content, "message", "content", ""),
                      }}
                  ...
                  {isSectionVisible(content, "signature") && (
                    <>
                      <p className="text-lg">
                        <strong className="text-foreground">
                          {getContent(content, "signature", "closing", "Avec toute ma gratitude et mon engagement,")}
                        </strong>
                      </p>
                      
                      <div className="pt-4">
                        <p className="text-foreground font-semibold text-lg">
                          {getContent(content, "signature", "name", "Annie Pichon")}
                        </p>
                        <p className="text-primary">
                          {getContent(content, "signature", "title", "Présidente de Reflet du Gabon")}
                        </p>
                      </div>
                    </>
                  )}
              ...
              {isSectionVisible(content, "cta") && (
                <div className="mt-12 text-center">
                  <p className="text-muted-foreground mb-6">
                    {getContent(content, "cta", "text", "Vous souhaitez nous rejoindre dans cette aventure ?")}
                  </p>
